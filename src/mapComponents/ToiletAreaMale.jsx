...
function ToiletAreaMale({ toiletCoordinate, toiletId, toiletName,toiletColor,isToiletEmpty,toiletGender,getToiletDataMale}) {

  const [toiletDataMale,setToiletDataMale] = useState();
  ...
  useEffect(()=>{
    ...
    axios.get("https://localhost:7118/api/bathroom?gender=m")
    .then(
      response => setToiletDataMale(response.data) // before setting ongoing reservations, sort them according to criteria
      )
    .catch(error => { 
      // what now? 
...
if(toiletDataMale == "Door open"){
  maleToiletData = true
}
else{
  maleToiletData = false
}
console.log("Erkekler Tuvaleti:"+maleToiletData)
  ...
  return (
    ...
    <Polygon 
      ...
      eventHandlers={{
        ...
        click:() =>{maleToiletData?setPopUpColor(".leaflet-popup-content-wrapper{width:200px;background-color:rgba(55, 114, 95, 1);}.leaflet-popup-tip{background-color:rgba(55, 114, 95, 1)}"):      setPopUpColor(".leaflet-popup-content-wrapper{width:200px;background-color:rgba(255, 168, 168, 1);}.leaflet-popup-tip{background-color:rgba(255, 168, 168, 1)}")}
